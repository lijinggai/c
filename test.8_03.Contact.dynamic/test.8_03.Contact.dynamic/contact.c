#include"contact.h"
void IntiContact(struct Contact* pc)//初始化
{
...
void AddContact(struct Contact* pc)//增加联系人信息
{
	CheckCapacity(pc);//判断容量是否满
	if (pc->size <pc->capacity)
	{
		printf("请输入名字:>");
...
void SaveContact(Contact* pc)//保存输入的通讯录中的信息
{
...
void LoadContact(Contact* pc)//载入
{
	...
	PeoInfo tmp = { 0 };
	while (fread(&(tmp), sizeof(PeoInfo), 1, pRead))//因为fread的返回值是实际的录入个数，如果录入失败返回0循环结束
	{
		CheckCapacity(pc);
		pc->date[pc->size] = tmp;
		pc->size++;
